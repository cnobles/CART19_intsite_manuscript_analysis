...
potential_genes <- list(
  ...
  "Composite" = unique(c(
    df1.2$Gene, df2.2$Gene, df3.2$Gene)))
...
go5 <- fisher_hyper_GO_test(
  potential_genes$Composite, all_GOBP_list, odds = FALSE, p_adjust = 'BH', 
  lower_tail = FALSE, cores = numCores
)

go5 <- dplyr::filter(
  go5, overlap_size >= min_GO_ovlp, GO_size <= max_GO_size, !is.na(GO_Term)
)

go5$grp <- cluster_by_list_similarity(
  keys = go5$GO_ID, keyList = all_GOBP_list, limitValues = df5$gene_name,
  cores = numCores
)

go5$grp.go <- membership(cluster_louvain(
  as.undirected(induced_subgraph(Gobp, go5$GO_ID))
))[go5$GO_ID]

kegg5 <- fisher_hyper_KEGG_test(
  potential_genes$Composite, k_pathList, odds = FALSE, p_adjust = 'BH', 
  lower_tail = FALSE, cores = numCores
)

kegg5 <- dplyr::filter(
  kegg5, overlap_size >= min_KEGG_ovlp, 
  KEGG_size <= max_KEGG_size, !is.na(KEGG_Term)
)

kegg5$grp <- cluster_by_list_similarity(
  keys = kegg5$path_id, keyList = k_pathList, limitValues = df5$gene_name, 
  cores = numCores
)

# Detailed table of genes identified meeting the specified criteria (>= 2)
df5.1 <- dplyr::filter(df5, count >= 2) %>%
...
criteria_order <- c("Enrichment", "Depletion", "Abundance", "Longitudinal")

### GO ----
slice_cnt <- pick_slice_cnt(go5$grp, lines_per_page)
...
go_criteria_list_df <- bind_rows(list(
    ...
    "Composite" = dplyr::select(go5, GO_ID, adj.p.value)),
  .id = "Criteria"
)
...
### KEGG ----
slice_cnt <- pick_slice_cnt(kegg5$grp, lines_per_page) 
...
kegg_criteria_list_df <- bind_rows(list(
    ...
    "Composite" = dplyr::select(kegg5, path_id, adj.p.value)),
  .id = "Criteria"
)

kegg_term_data <- kegg5 %>%
  dplyr::mutate(
...
df6_list <- lapply(sup_table_split, function(index){
  cols <- 49 + index
  df6[sapply(seq_len(nrow(df6)), function(i) all(as.logical(df6[i,cols]))),]
})

names(df6_list) <- sapply(sup_table_split, function(index){
  paste(c("Enrichment", "Depletion", "Abundance", "Longitudinal")[
    index], collapse = " - ")
})

df6_list <- df6_list[sapply(df6_list, nrow) > 0]

output_impact <- dplyr::select(
    df5, -TDN_sum_abund, -TP_sum_abund, -count, -tcount, -TDN_num_pats_CR,
...
\newpage
```{r summary_tbl}
sig_onco <- p.adjust(sapply(seq_len(nrow(df4)), function(i){
  mat <- matrix(c(
    df4$gene_count[i] * df4$onco_genes[i] / 100,
    df4$gene_count[i] * (1 - (df4$onco_genes[i] / 100)),
...
sig_tumSup <- p.adjust(sapply(seq_len(nrow(df4)), function(i){
  mat <- matrix(c(
    df4$gene_count[i] * df4$tumor_sups[i] / 100,
    df4$gene_count[i] * (1 - (df4$tumor_sups[i] / 100)),
...
```{r summary_tbl2}
kable(
    df5.1, format = "latex", booktabs = TRUE,
...
go5 %>%
  ...
  kable(
    ...
    caption = paste0(
      ...
      length(which(unique(df5$gene_name) %in% all_GOBP_df$gene_sym)))) %>%
...
kegg5 %>%
  ...
  kable(
    ...
    caption = paste0("KEGG Pathway analysis. Top ", slice_cnt, " per group.",
        " Total genes considered: ", 
        length(which(unique(df5$gene_name) %in% k_path_df$gene_sym)))) %>%
  kableExtra::kable_styling(
    latex_options = "hold_position", full_width = TRUE, font_size = 10) %>%
