...
### Load cluster gene information
cluster_genes <- read.csv(
  file.path(outputDir, "cart19_cluster_gene_info.csv")
)

cluster_gene_list <- cluster_genes$gene_name
...
### Load stats about genes identified by clones 
gene_stats <- read.csv(
...
gene_impact <- readRDS(
  ...
  dplyr::filter(gene_name != "TET2-AS1") %>%
...
all_sites <- all_sites[all_sites$specimen %in% summaries$specimen$specimen]
...
---
## Figure 2. Longitudinal evaluation of inferred population sizes of CAR19-transduced T cells.
...
## Figure 2. Markers of insertional mutagenesis and clonal expansion in CAR19-modified cells.
...
grid.arrange(
  ...
  nrow = 1)
```
Figure 3ABC. A-C) Rank-ordered plots display the abundance of each identified clone from their respective groups – transduction (TDN) products, CR/PRtd – Day 28, and PR/NR – Day 28. Highly abundant clones were determined by identifying a cutoff abundance value for the top 1% of all expanded clones within patients, cutoff = 9 cells. Clones at or above this cutoff are colored red, and the top 10 most abundant clones are labeled with their proximal gene symbol.

```{r fig2D, fig.width=4, fig.height=3}
gene_freq_bivar <- within(
  list(), {
    ...
    plot <- ggplot() +
      ...
      geom_point(
        data = dplyr::filter(freq_bivar, freq_fisher_test <= 0.05, pct_chg > 0),
      ...
      geom_point(
        ...
        shape = 21, color = "red") +
      geom_text_repel(
        data = dplyr::filter(freq_bivar, gene_name %in% key_gois, pct_chg >= 0),
...
go_kegg_heatmaps <- within(
  list(), {
    ...
    criteria_order <- c("Depletion", "Enrichment", "Abundance", "Longitudinal")
...
specimen_population_summary <- within(
  list(), {
    ...
    chao1_sig <- as.data.frame(
      ...
      separate(V1, into = c("response1", "timepoint1"), sep = " - ") %>%
      separate(V2, into = c("response2", "timepoint2"), sep = " - ") %>%
      dplyr::filter(
        ifelse(
          response1 == response2, 
...
## Figure 5. Predicting clinical outcome from transduction products.
```{r model_setup}
model_analysis <- within(
  list(), {
    ...
    # that separate two or more classes given a dimensional input dataset 
...
```{r model_heatmap, eval=FALSE}
model_heatmap <- within(
  model_analysis, {
    
    # Heatmap for tdn model ----
    ...
    coef_lasso_tdn <- apply(coef_lasso_tdn, 2, function(x) -x / sum(abs(x)))
    colnames(coef_lasso_tdn) <- "tdn_lasso_model"
    
    wt_tdn_mod_composite <- rbind(
      t(coef_lasso_tdn),
      wt_pca_cont_tdn[,seq_len(tdn_pca_cutoff)])

    tdn_heatmap_data <- as.data.frame(wt_tdn_mod_composite) %>%
      dplyr::mutate(
        metric = row.names(.),
        feat_grp = names(grouped_features)[match(metric, grouped_features)],
        feat_grp = ifelse(
          is.na(feat_grp) & grepl("model", metric), "Model", feat_grp),
        feat_grp = factor(
          feat_grp, 
          levels = c(
            "Model", "Population", "Genomic", "Epigenetic", "Bound Proteins"
          )
        )
      ) %>%
      dplyr::arrange(feat_grp, desc(PC1)) %>%
      tidyr::gather("PC", "loading", -feat_grp, -metric) %>%
      dplyr::mutate(
        metric = factor(metric, levels = rev(unique(metric))),
        PC = factor(PC, levels = paste0("PC", seq_len(tdn_pca_cutoff)))) %>%
      dplyr::filter(feat_grp == "Model" & loading != 0 | feat_grp != "Model")
    
    # 7.5"x10"
    tdn_loading_heatmap <- ggplot(tdn_heatmap_data, aes(x = PC, y = metric)) +
      geom_tile(aes(fill = loading), color = "black") +
      facet_grid(feat_grp ~ ., scales = "free", space = "free") +
      scale_x_discrete(position = "top") +
      scale_fill_distiller(palette = "RdBu", direction = -1) +
      guides(fill = guide_colorbar(
        title.position = "left", title.hjust = 0.5, direction = "horizontal"
      )) +
      labs(x = "Principle Components", y = "Features", fill = "Loadings") +
      custom_theme + 
      theme(
        legend.position = "bottom",
        strip.placement = "outside",
        strip.text.y = element_text(angle = 0),
        axis.text.x.top = element_text(
          angle = 90, hjust = 0, vjust = 0.5, size = 7),
        axis.text.y = element_text(size = 7),
        axis.line = element_blank(),
        axis.ticks.y = element_blank(),
        aspect.ratio =
          n_distinct(tdn_heatmap_data$PC)/n_distinct(tdn_heatmap_data$metric)
      )
    
    # Heatmap for d28 model ----
    ...
    coef_lasso_d28 <- apply(coef_lasso_d28, 2, function(x) -x / sum(abs(x)))
    colnames(coef_lasso_d28) <- "d28_lasso_model"
    
    wt_d28_mod_composite <- rbind(
      t(coef_lasso_d28),
      wt_pca_cont_d28[,seq_len(d28_pca_cutoff)])

    d28_heatmap_data <- as.data.frame(wt_d28_mod_composite) %>%
      dplyr::mutate(
        metric = row.names(.),
        feat_grp = names(grouped_features)[match(metric, grouped_features)],
        feat_grp = ifelse(
      ...
      dplyr::mutate(
        metric = factor(metric, levels = rev(unique(metric))),
        PC = factor(PC, levels = paste0("PC", seq_len(d28_pca_cutoff)))) %>%
      dplyr::filter(feat_grp == "Model" & loading != 0 | feat_grp != "Model")
    
    d28_loading_heatmap <- ggplot(d28_heatmap_data, aes(x = PC, y = metric)) +
      geom_tile(aes(fill = loading), color = "black") +
      facet_grid(feat_grp ~ ., scales = "free", space = "free") +
      scale_x_discrete(position = "top") +
      scale_fill_distiller(palette = "RdBu", direction = -1) +
      ...
      theme(
        ...
        axis.ticks.y = element_blank(),
        aspect.ratio =
          n_distinct(d28_heatmap_data$PC)/n_distinct(d28_heatmap_data$metric)
      )
...
```{r fig5A, fig.height=3.5, fig.width=7}
model_analysis <- within(
  model_analysis, {
...
```{r fig5CD, fig.height=6, fig.width=7.5}
# Model contributions
model_analysis <- within(
  model_analysis, {
...
sup_tbl_s1 <- patient_data %>%
  dplyr::select(patient, disease, clin_trial, response, determinant_response) %>%
