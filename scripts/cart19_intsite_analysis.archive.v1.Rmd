...
shifrut_comparison <- within(
  list(), {
    ...
    gene_set_list <- c(potential_genes, list("Composite" = composite_genes))
    
    # Compare to the top 25 genes from each gene set

    stats_tbl <- dplyr::bind_rows(lapply(
          gene_set_list, 
          function(genes, pos_genes, neg_genes, total_considered){
          
            # Normalize lists
            genes <- genes[genes %in% total_considered]
            not_genes <- total_considered[!total_considered %in% genes]
            pos_genes <- pos_genes[pos_genes %in% total_considered]
            not_pos_genes <- total_considered[!total_considered %in% pos_genes]
            neg_genes <- neg_genes[neg_genes %in% total_considered]
            not_neg_genes <- total_considered[!total_considered %in% neg_genes]
            
            # Generate matrices
            ## Positive
            pos_mat <- matrix(
              c(
                length(intersect(genes, pos_genes)),
                length(intersect(not_genes, pos_genes)),
                length(intersect(genes, not_pos_genes)),
                length(intersect(not_genes, not_pos_genes))
              ),
              ncol = 2
            )
            ...
            neg_mat <- matrix(
              c(
                length(intersect(genes, neg_genes)),
                length(intersect(not_genes, neg_genes)),
                length(intersect(genes, not_neg_genes)),
                length(intersect(not_genes, not_neg_genes))
              ),
              ncol = 2
            )
            ...
            data.frame(
              ...
              "pos.size" = length(pos_genes),
              "pos.ovlp" = length(intersect(genes, pos_genes)),
              "pos.odds" = fisher.test(pos_mat)$estimate,
              "pos.pval" = fisher.test(pos_mat)$p.value,
              "neg.size" = length(neg_genes),
              "neg.ovlp" = length(intersect(genes, neg_genes)),
              "neg.odds" = fisher.test(neg_mat)$estimate,
              "neg.pval" = fisher.test(neg_mat)$p.value
            )
      ...
      dplyr::mutate(
        pos.pval = p.adjust(pos.pval, method = "BH"),
        neg.pval = p.adjust(neg.pval, method = "BH")
      )
      
    
    neg_plot <- ggplot() +
      geom_point(
        data = dplyr::filter(shifrut_rep_screen, `neg|p-value` < 0.1),
        aes(x = `neg|rank`, y = log(`neg|score`))
      ) +
      geom_segment(
        data = dplyr::filter(
          shifrut_rep_screen, 
          `neg|p-value` < 0.1, 
          id %in% gene_set_list$Depletion
        ),
        aes(x = `neg|rank`, xend = `neg|rank`, y = log(`neg|score`), yend = 0),
        color = "red"
      ) +
      geom_segment(
        data = dplyr::filter(
          shifrut_rep_screen, 
          `neg|p-value` < 0.1, 
          id %in% gene_set_list$Composite
        ),
        aes(x = `neg|rank`, xend = `neg|rank`, y = log(`neg|score`), yend = 0),
        color = "blue"
      ) +
      scale_x_log10() +
      labs(x = "Negative Rank", y = "Log(Score)") +
      custom_theme
    
    pos_plot <- ggplot() +
      geom_point(
        data = dplyr::filter(shifrut_rep_screen, `pos|p-value` < 0.1),
        aes(x = `pos|rank`, y = -log(`pos|score`))
      ) +
      geom_segment(
        data = dplyr::filter(
          shifrut_rep_screen, 
        ...
        aes(x = `pos|rank`, xend = `pos|rank`, y = -log(`pos|score`), yend = 0),
        color = "red"
      ) +
      geom_segment(
        data = dplyr::filter(
      ...
      scale_x_log10() +
      labs(x = "Positive Rank", y = "-Log(Score)") +
      custom_theme
...
grid.arrange(
  grobs = list(shifrut_comparison$neg_plot, shifrut_comparison$pos_plot),
  nrow = 1
)
