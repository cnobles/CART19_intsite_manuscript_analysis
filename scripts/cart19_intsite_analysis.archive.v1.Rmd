...
```{r gene_expression_comparision, fig.height=6, fig.width=4}
gen_express <- within(
  list(), {
    ...
    pct_intsites_by_grp_plot <- ggplot() +
      ...
      geom_text(
        ...
        aes(
          ...
          y = rep(12.5, nrow(pat_spec_sig_test)),
    ...
    out_plot <- grid.arrange(
      grobs = list(
        total_gene_exp_plot, pct_intsites_by_grp_plot
      ),
...
long_clone_plots <- within(
  list(), {
    ...
    clone_data <- as.data.frame(cond_uniq_sites, row.names = NULL) %>%
      filter(patient %in% std_clin_patients) %>%
      select(
        patient, timepoint, celltype, specimen, 
        estAbund, relAbund, posid, gene_id
      ) %>%
      mutate(dtime = convert_time(as.character(timepoint))) %>%
      group_by(patient, posid, gene_id) %>%
      summarise(
      ...
      arrange(desc(num_tp), desc(max_ab), desc(min_ab)) %>%
      ungroup()
    
    # Isolate data for plot
    plot_clones <- clone_data %>%
      group_by(patient) %>%
      arrange(desc(max_tp - min_tp), desc(max_rb)) %>%
      slice(1:10) %$%
      unique(paste0(patient, ":", posid))

    plot_data <- as.data.frame(cond_uniq_sites, row.names = NULL) %>%
      select(patient, timepoint, posid, gene_id, estAbund, relAbund) %>%
      filter(paste0(patient, ":", posid) %in% plot_clones) %>%
      mutate(
      ...
      group_by(patient, timepoint, tp, pf, posid, gene_id) %>%
      summarise(
      ...
      group_by(patient, posid, gene_id) %>%
      mutate(num_tp = n_distinct(tp)) %>%
      ungroup() %>%
      group_by(num_tp) %>%
      mutate(pf = as.integer(factor(pf, levels = sort(unique(pf))))) %>%
      ungroup
